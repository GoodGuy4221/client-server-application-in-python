    ...
    def main(self):
        if '-p' in argv:
            try:
                ...
                sys.stdout.write('после параметра -\'p\' необходимо указать номер порта.')
                exit(1)
            except ValueError:
                sys.stdout.write(f'в качестве порта может быть указано только число в диапазоне от {MIN_PORT_NUMBER} до '
                      f'{MAX_PORT_NUMBER}.')
                exit(1)
